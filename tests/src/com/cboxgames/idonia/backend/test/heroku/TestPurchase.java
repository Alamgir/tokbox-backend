...
public class TestPurchase extends HttpTest {
	private static String _purchase_uri = "/purchases/details"; // HTTP Method: GET
	
	public TestPurchase(HttpTestRunnable callback_finished, long test_timeout) {
	...
	public void run() {
		try {
			...
			HttpGet get = new HttpGet(new URI(IdoniaBackendTest.SERVER_ROOT + _purchase_uri));
			...
			Class<PurchaseWrapper[]> class_type = PurchaseWrapper[].class;
			PurchaseWrapper[] unw = IdoniaBackendTest.json_converter.getObject(data, class_type);
			
			// Do some checking here.
			assert unw != null : "Returned Purchase object was null.  Our model is probably missing some fields.";
