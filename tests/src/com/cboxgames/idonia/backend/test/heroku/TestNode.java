...
public class TestNode extends HttpTest {
	private static String _node_uri = "/nodes/details"; // HTTP method: GET
	
	public TestNode(HttpTestRunnable callback_finished, long test_timeout) {
	...
	public void run() {
		try {
			...
			HttpGet get = new HttpGet(new URI(IdoniaBackendTest.SERVER_ROOT + _node_uri));
			...
			Class<NodeWrapper[]> class_type = NodeWrapper[].class;
			NodeWrapper[] nw = IdoniaBackendTest.json_converter.getObject(data, class_type);
			
			// Do some checking here.
			assert nw != null : "Returned Node object was null.  Our model is probably missing some fields.";
