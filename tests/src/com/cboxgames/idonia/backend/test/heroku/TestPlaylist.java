...
public class TestPlaylist extends HttpTest {
	private static String _playlist_uri = "/playlists/details"; // HTTP Method: GET
	
	public TestPlaylist(HttpTestRunnable callback_finished, long test_timeout) {
	...
	public void run() {
		try {
			...
			HttpGet get = new HttpGet(new URI(IdoniaBackendTest.SERVER_ROOT + _playlist_uri));
			...
			Class<PlaylistWrapper[]> class_type = PlaylistWrapper[].class;
			PlaylistWrapper[] unw = IdoniaBackendTest.json_converter.getObject(data, class_type);
			
			// Do some checking here.
			assert unw != null : "Returned Playlist object was null.  Our model is probably missing some fields.";
