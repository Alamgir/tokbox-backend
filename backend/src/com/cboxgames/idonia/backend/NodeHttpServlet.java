...
/**
 * Handle "/nodes/details"
 * HTTP Method: GET
 * @author Michael Chang
 *
 */
@SuppressWarnings("serial")
public class NodeHttpServlet extends HttpServlet {

    private SqlDataSource _sql_data_source;
	private NodeDBSQL _db_sql;
	...
	public void init() {
        ...
        try {
            _db_sql = new NodeDBSQL(_sql_data_source.get_data_source(), this.getServletContext());
        }
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response)
        throws IOException, ServletException {
      	...
      	String uri_str = request.getRequestURI();
       	if (UriToArgv.verifyUrl(uri_str, "nodes", "details") == false) {
        ...
        ResponseTools.prepareResponseJson(response, _mapper, _db_sql.getNodeDetails(), Constants.SC_OK);
    }
}