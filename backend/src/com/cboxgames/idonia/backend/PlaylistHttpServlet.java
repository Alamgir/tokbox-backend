...
/**
 * Handle "/playlists/details"
 * HTTP Method: GET
 * @author Michael Chang
 *
 */
@SuppressWarnings("serial")
public class PlaylistHttpServlet extends HttpServlet {

	private SqlDataSource _sql_data_source;
	private PlaylistDBSQL _db_sql;
	...
	public void init() {
        ...
        try {
            _db_sql = new PlaylistDBSQL(_sql_data_source.get_data_source(), this.getServletContext());
        }
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response)
    	...
    	String uri_str = request.getRequestURI();
       	if (UriToArgv.verifyUrl(uri_str, "playlists", "details") == false) {
        ...
        ResponseTools.prepareResponseJson(response, _mapper, _db_sql.getPlaylistDetails(), Constants.SC_OK);
    }
}