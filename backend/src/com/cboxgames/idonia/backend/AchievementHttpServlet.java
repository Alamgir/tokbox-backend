...
/**
 * Handle "/achievements/details"
 * HTTP Method: GET
 * @author Michael Chang
 *
 */
@SuppressWarnings("serial")
public class AchievementHttpServlet extends HttpServlet {

	private SqlDataSource _sql_data_source;
	private AchievementDBSQL _db_sql;
	...
	public void init() {
        ...
        try {
            _db_sql = new AchievementDBSQL(_sql_data_source.get_data_source(), this.getServletContext());
        }
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response)
    	...
    	String uri_str = request.getRequestURI();
       	if (UriToArgv.verifyUrl(uri_str, "achievements", "details") == false) {
    	...
    	ResponseTools.prepareResponseJson(response, _mapper, _db_sql.getAchievementDetails(), Constants.SC_OK);
	}
}